...
| V2-E3 Real Match Flow and Session Robustness | partial | two-player queue pairing, reconnect latest snapshot 복구, delayed/duplicate reconciliation은 완료, fanout hardening이 남아 있음 |
...
이번 readiness 재점검은 delayed/duplicate event reconciliation 보강과 docs update를 함께 반영합니다.
...
| Story status review | PASS | `V2-E3-ST03` 완료 상태 반영 |

## 다음 구현 순서

스킬 domain source가 아직 없으므로 다음 안전한 구현 단위는 승인된 기존 runtime boundary 안에서 진행합니다.

1. `V2-E3-ST04`: timeout watcher와 surrender event fanout 안정화.
2. `V2-E1-ST02`: concrete frame recognizer runtime 선택 및 adapter 결합은 runtime 선택이 승인된 뒤 진행.
3. `V2-E1-ST04`: recognizer restart, cleanup, permission recovery hardening은 runtime 선택 이후 진행.
