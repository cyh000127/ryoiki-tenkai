...
- full v2 feature release 판정: 스킬 도메인 승인 전까지 준비 미완료
- 릴리스 판단 근거:
  - 완료된 v2 hardening 항목은 implementation record와 smoke checklist에 연결되어 있습니다.
  - recognizer runtime binding은 browser frame signal runtime으로 완료되었습니다.
  - blocked 항목이 v2 story와 smoke checklist에서 분리되어 있습니다.
...
| Browser frame signal runtime binding | PASS | `docs/implementation-artifacts/v2-12-browser-frame-signal-runtime.ko.md` |
...
v2의 스킬 도메인 blocked 항목은 임의 구현하지 않고 v3 handoff 안정화 범위로 이어졌습니다. recognizer runtime binding은 이후 `v2-12`에서 완료되었습니다.
...
| Epic | 현재 상태 | 비고 |
| --- | --- | --- |
| V2-E1 Live Recognition Runtime Hardening | done | adapter boundary, camera smoke, no-hand/unstable/recognized UI 분리, restart/cleanup hardening, runtime port boundary, fake runtime port smoke, browser frame signal runtime binding이 완료됨 |
...
이번 readiness 재점검은 browser frame signal runtime binding과 docs update를 함께 반영합니다.
...
| Story status review | PASS | `V2-E1-ST02`는 done, `V2-E4`는 blocked 유지 |

## 다음 구현 순서

스킬 domain source가 아직 없으므로 다음 구현 단위는 승인 후 진행합니다.

1. `V2-E4-ST01`: skill domain source format 정의는 approved source가 생긴 뒤 진행.
2. `V2-E4-ST02`부터 `V2-E4-ST04`: approved source를 fixture, contract, frontend rendering에 반영.
