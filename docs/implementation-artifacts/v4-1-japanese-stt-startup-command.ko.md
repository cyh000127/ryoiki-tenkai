# v4-1 일본어 음성 시동 명령 구현 기록

## 목적

무료로 빠르게 적용 가능한 일본어 음성 시동 명령을 홈 화면에 추가한다. 음성 명령은 앱 시작 흐름을 돕는 entry command이며, 전투 스킬 발동이나 gesture sequence 판정과는 분리한다.
...
- 미지원, 권한 거부, 실패 상태에는 수동 시동 fallback을 제공한다.
...
- 사용자가 홈 화면에서 음성 시동 버튼을 누를 수 있다.
- 일본어 인식 결과가 화면에 표시된다.
- 등록 명령어가 transcript에 포함되면 시동 성공으로 처리된다.
- 시동 성공은 현재 플레이어 상태에 맞춰 게스트 생성, 로드아웃 이동, 매칭 시작 중 하나로 연결된다.
- 인식 실패 또는 미지원 상태에서도 수동 시동이 가능하다.
- 외부 유료 STT를 사용하지 않는다.
