...
- [ ] presentation-set list가 선택 가능한 metadata를 반환한다.
...
- [ ] 위 critical smoke item이 모두 통과하거나 blocker가 문서화되어 있다.
- [ ] known gap이 MVP exclusion 또는 follow-up work로 표시되어 있다.
- [ ] 이 체크리스트 업데이트만으로 FE, BE, 입력 런타임 source change가 필요하지 않다.
