...
- [x] camera permission allowed 상태가 camera-ready와 hand-detected feedback을 보여준다.
  자동화 근거: `pnpm --dir FE/app smoke:camera`, `BattleGameWorkspace.test.tsx`
- [x] camera permission denied 상태가 명확한 blocked state를 보여주고 battle action submission으로 들어가지 않는다.
  자동화 근거: `pnpm --dir FE/app smoke:camera`, `BattleGameWorkspace.test.tsx`
- [x] no-hand, unstable-hand, recognized-gesture state가 시각적으로 구분된다.
  자동화 근거: `FE/app/tests/unit/BattleGameWorkspace.test.tsx`, `docs/implementation-artifacts/v2-4-recognition-ui-state.ko.md`
