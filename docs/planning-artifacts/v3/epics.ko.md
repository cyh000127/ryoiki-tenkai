...
이 문서는 v3를 릴리스/인수인계 안정화 범위로 정의합니다. v2에서 blocked로 남은 concrete frame recognizer runtime 선택과 skill/resource 구현은 v3 범위에 포함하지 않습니다.
...
승인되지 않은 domain/runtime 결정을 구현으로 착각하지 않도록 blocker를 유지합니다.

### 수용 신호

- concrete runtime 선택 전까지 recognizer runtime binding은 blocked로 남습니다.
