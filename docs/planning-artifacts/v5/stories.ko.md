...
### V5-E4-ST02: MediaPipe 기본 runtime 반영

- Status: done
- Scope: 기본 손동작 runtime을 browser frame signal에서 MediaPipe Hand Landmarker 기반 heuristic 분류기로 교체하고, frame signal runtime은 테스트/명시적 fallback으로만 유지한다.
- Acceptance criteria: 기본 recognizer가 MediaPipe runtime을 사용하고, `no_hand`/`unstable`/`recognized` 상태가 유지된다.
- Dependencies: `@mediapipe/tasks-vision`, 기존 live recognizer lifecycle.
- Verification: live recognizer unit test, workspace camera regression test.
