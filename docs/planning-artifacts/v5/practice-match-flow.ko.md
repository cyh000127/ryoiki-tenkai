...
현재 화면에는 `홈`, `로드아웃`, `연습모드`, `매칭`, `전투`, `결과`, `전적`이 같은 탭 위계로 노출되어 있다. 하지만 실제 제품 흐름에서 `매칭`, `전투`, `결과`는 사용자가 자유롭게 들어가는 목적지가 아니라, 버튼과 서버 이벤트에 의해 진행되는 런타임 상태다.

v5의 목표는 사용자가 "어디를 눌러야 다음 단계로 가는지"를 고민하지 않도록 연습모드와 매칭의 관계를 다시 설계하는 것이다.

## 현재 위치와 문제

현재 연습모드는 왼쪽 탭의 `연습모드`에 있으며, 홈 화면 내부의 `연습모드 시작` 버튼으로도 이동할 수 있다. 이 구조는 기능적으로 존재하지만, 탭 이동과 내부 버튼 이동이 같은 역할을 하면서 플로우가 불명확해진다.
...
v5에서는 왼쪽 탭을 `홈`, `로드아웃`, `연습`, `전적` 중심으로 축소한다. `매칭 중`, `전투 중`, `결과`는 탭이 아니라 상단 진행 표시와 상태 화면으로 표현한다.
...
| 항목 | 현재 | v5 목표 |
| --- | --- | --- |
| 왼쪽 탭 | 홈, 로드아웃, 연습모드, 매칭, 전투, 결과, 전적 | 홈, 로드아웃, 연습, 전적 |
| 매칭 진입 | 탭과 버튼이 혼재 | CTA 이후 진행 상태로만 진입 |
| 전투 진입 | 탭에 노출됨 | 서버 이벤트 이후 자동 진입 |
| 결과 진입 | 탭에 노출됨 | 전투 종료 이후 자동 진입 |
| 연습 위치 | 탭과 홈 버튼 모두 존재 | 목적지 탭 + 명확한 홈 보조 CTA |
| 연습 설명 | 서버 전투와 분리됨은 표시하지만 위계가 약함 | 레이팅/전적/매칭 로드아웃과 무관함을 명확히 표시 |

## 결정

v5 구현 전에는 이 문서를 기준으로 프론트 정보구조를 먼저 수정한다. 기능을 추가하기 전에 `사용자 목적지`와 `시스템 진행 상태`를 분리하고, 연습과 매칭이 서로 다른 역할을 가진다는 점을 UI와 테스트에 고정한다.
