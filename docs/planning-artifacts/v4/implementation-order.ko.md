# v4 구현 순서

## 1. 명령어 모델

- 브라우저 speech recognition wrapper를 공용 transcript recognizer port로 분리한다.
- `ja-JP` 시작 명령 모델은 공용 STT port를 사용한다.
- 기본 명령어 배열과 정규화 함수를 작성한다.
- 키워드 포함 매칭을 단위 테스트로 고정한다.

## 2. 홈 화면 통합

- 홈 화면에 음성 시작 패널을 추가한다.
- 상태는 `idle`, `listening`, `matched`, `rejected`, `unsupported`, `blocked`, `error`로 분리한다.
- 성공 시 기존 handler를 재사용한다.
  - 세션 없음: 게스트 생성
  - 로드아웃 미저장: 로드아웃 화면
  - 준비 완료: 매칭 시작

## 3. 마이크 없는 시작과 예외 처리

- 미지원 또는 권한 거부 상태를 화면에 표시한다.
- 마이크 없이 시작 버튼을 항상 제공한다.
- transcript와 matched command는 UI 상태에만 유지한다.

## 4. 문서와 검증

- v4 계획 문서와 구현 기록을 한국어/영어로 추가한다.
- README에 v4 링크와 현재 상태를 추가한다.
- frontend typecheck, unit test, build를 실행한다.

## 5. 추후 손동작/스킬 연결 기준

- STT는 trigger phrase 인식까지만 담당한다.
- trigger 성공 후 손동작 입력을 활성화한다.
- 손동작 sequence와 skill action 판정은 approved skill domain source가 준비된 뒤 구현한다.

## 6. 캐릭터/스킬 후보 정리

- Phase 1은 주술회전 고유 캐릭터/술식명으로 시작한다.
- 주술회전 캐릭터, 일반 스킬, 영역전개, STT trigger 후보를 제품 카탈로그 문서에 정리한다.
- 오리지널 캐릭터 후보는 추후 확장 또는 대체 후보로 별도 유지한다.
- 손모양 내부 token과 실제 수치는 이후 승인 단계에서 확정한다.

## 커밋 단위

- 권장 커밋: `feat(frontend): 일본어 음성 시작 명령 추가`
- 포함 범위: docs, frontend model, home UI, tests.
- 이유: 한 기능의 문서/구현/검증을 한 단위로 추적해 push 수를 줄인다.
